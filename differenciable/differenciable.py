...
class DifferenciableCore:
    ...
    def backpropagation(self):
        ...
        for i in range(self.TOTAL_STEPS - 2, -1, -1):
            eq_vec = (self.TimeStep * dg_dx[i+1] + dg_dv[i+1])

            sys_solve = self.solve_system(self.eq_mat[i+1], eq_vec) # i+1

            dg_dx[i] += (self.TimeStep * self.df_dx[i+1]).dot(sys_solve) + dg_dx[i+1]
            dg_dv[i] += self.mass_matrix.dot(sys_solve)
